...
class VacancyControllerTest {
    ...
    public void whenRequestVacancyByIdThenGetVacancyPageWithCities() {
        int vacancyId = 1;
        var expectedVacancy = new Vacancy(vacancyId, "test1", "desc1", now(), true, 1, 2);
        var city1 = new City(1, "Москва");
        var city2 = new City(2, "Санкт-Петербург");
        var expectedCities = List.of(city1, city2);
        when(cityService.findAll()).thenReturn(expectedCities);
        when(vacancyService.findById(vacancyId)).thenReturn(Optional.of(expectedVacancy));
    ...
    @Test
    public void whenRequestNotExistVacancyByIdThenGetErrorMessage() {
    ...
    @Test
    public void whenUpdateVacancyIsSuccess() throws IOException {
        var vacancy1 = new Vacancy(1, "test1", "desc1", now(), true, 1, 2);
        var fileDto = new FileDto(testFile.getOriginalFilename(), testFile.getBytes());
        when(vacancyService.update(vacancy1, fileDto)).thenReturn(true);
    ...
    @Test
    public void whenUpdateNotExistVacancyThenGetErrorMessage() {
        String expectedMessage = "Вакансия с указанным идентификатором не найдена";
    ...
    @Test
    public void whenDeleteVacancyIsSuccess() {
        int vacancyId = 1;
    ...
    @Test
    public void whenDeleteNotExistVacancyThenGetErrorMessage() {
        String expectedMessage = "Вакансия с указанным идентификатором не найдена";
        int vacancyId = 1;

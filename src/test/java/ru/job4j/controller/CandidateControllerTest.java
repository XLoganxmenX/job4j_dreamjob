...
class CandidateControllerTest {
    ...
    public void whenPostCandidateWithFileThenSameDataAndRedirectToCandidatesPage() throws Exception {
        ...
        when(candidateService.save(
                candidateArgumentCaptor.capture(), fileDtoArgumentCaptor.capture())
    ...
    @Test
    public void whenCreateAndSomeExceptionThrownThenGetErrorPageWithMessage() {
    ...
    public void whenRequestCandidateByIdThenGetCandidatePageWithCities() {
        int candidateId = 1;
        var expectedCandidate = new Candidate(candidateId, "test1", "desc1", now(), 1, 2);
        var city1 = new City(1, "Москва");
        var city2 = new City(2, "Санкт-Петербург");
        var expectedCities = List.of(city1, city2);
        when(cityService.findAll()).thenReturn(expectedCities);
        when(candidateService.findById(candidateId)).thenReturn(Optional.of(expectedCandidate));
    ...
    @Test
    public void whenRequestNotExistCandidateByIdThenGetErrorMessage() {
        String expectedMessage = "Резюме с указанным идентификатором не найдено";
    ...
    @Test
    public void whenUpdateCandidateIsSuccess() throws IOException {
        var candidate = new Candidate(1, "test1", "desc1", now(), 1, 2);
        var fileDto = new FileDto(testFile.getOriginalFilename(), testFile.getBytes());
        when(candidateService.update(candidate, fileDto)).thenReturn(true);
    ...
    @Test
    public void whenUpdateNotExistCandidateThenGetErrorMessage() {
        String expectedMessage = "Резюме с указанным идентификатором не найдено";
    ...
    @Test
    public void whenUpdateAndSomeExceptionThrownThenGetErrorPageWithMessage() {
        var expectedException = new RuntimeException("Failed to write file");
    ...
    @Test
    public void whenDeleteCandidateIsSuccess() {
        int candidateId = 1;
    ...
    @Test
    public void whenDeleteNotExistCandidateThenGetErrorMessage() {
        String expectedMessage = "Резюме с указанным идентификатором не найдено";
        int candidateId = 1;
